...
const CRIMEAN_TATAR_RO_ALPHABET_PRONUNCIATION_GUIDE = `
...
const CRIMEAN_TATAR_RO_SCT_DT_SUMMARY_INFO = `
...
const CRIMEAN_TATAR_RO_BIRD_NAMES_TABLE = `
...
const CRIMEAN_TATAR_RO_COMPREHENSIVE_GRAMMAR_DETAILS = `
...
export const translateText = async (
  ...
  // Lookup 1: Any language -> Crimean Tatar (RO)
  // (Primarily effective for Romanian -> Crimean Tatar (RO) due to dictionary content)
  if (targetLangName === 'Crimean Tatar (Romania)') {
    ...
    if (searchTextLower) {
      for (const entry of PARSED_DICTIONARY_ENTRIES) {
        // entry.details contains the Romanian definitions and other info.
        // Example: "adj. posibil; probabil; admisibil. // ● exemplu."
        const definitionPart = entry.details.split('//')[0].trim(); // Get the part before examples
        ...
        for (const defWithPossibleAnnotations of definitionsFromEntry) {
          ...
          // Remove trailing period if it's the last char of the segment
          if (currentDef.endsWith('.')) {
            currentDef = currentDef.slice(0, -1);
          }
          
          // Remove known POS prefixes.
          ...
          // Remove parenthetical annotations like (zool.), (bot.), (fig.), (lat., ...) from the start of the remaining string
  ...
  // Lookup 2: Crimean Tatar (RO) -> Romanian (Existing logic)
  if (sourceLangName === 'Crimean Tatar (Romania)' && targetLangName === 'Romanian') {
    ...
    if (searchTextLower) {
      ...
      if (dictionaryMatch) {
        // Original format:
        return `(Din dicționar)\nTermen: ${dictionaryMatch.term}\nDetalii: ${dictionaryMatch.details}`;
        // Simplified format if preferred (just Romanian part):
        // const romanianDefinitions = dictionaryMatch.details.split('//')[0].trim();
        // return `(Din dicționar) ${romanianDefinitions}`;
      }
    }
  }
  ...
  try {
    ...
    console.error("Error refining text with Gemini API (Refinement Pass):", error);
     if (error instanceof Error) {
        ...
        // If refinement fails, it might be better to return the initial translation than nothing.
